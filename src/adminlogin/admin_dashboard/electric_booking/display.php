...
<html>
<head>
  <title>Bookings for Electrical Services</title>
  <link rel="stylesheet" type="text/css" href="style.css">
</head>
<body>
  <div class="table-container">
    <h2>Bookings for Electrical Services</h2>
    <table>
      <thead>
        <tr>
          <th>Name</th>
      ...
      <tbody>
        <?php
          ...
          $sql = "SELECT * FROM electric_details";
  ...
  function updateStatus(name, address, status) {
    ...
    xhr.open("GET", "../clean_bookings/update_status.php?name=" + encodeURIComponent(name) + "&address=" + encodeURIComponent(address) + "&status=" + encodeURIComponent(status), true);
    xhr.send();
  }
